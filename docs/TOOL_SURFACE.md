...
| `retrieve_tool_result` | Read summaries or slices of prior large tool outputs spilled to `~/.codewhale/tool_outputs/` (legacy `~/.deepseek/tool_outputs/` remains readable); use `summary`, `head`, `tail`, `lines`, or `query` instead of replaying the whole result. |
