...
const FALLBACK_DISPATCH_EN: CuratedDispatch = {
  ...
  headline: "v0.8.48 release hardening — agent liveness, MCP discovery, IME input, and Windows launch polish.",
  summary:
    "This dispatch is regenerated by DeepSeek V4-Flash on a six-hour cron. Until the cron fills in fresh repo activity, the static links below point at the current release-ready surfaces.",
  highlights: [
    { title: "Install", href: "/install", tag: "0.8.48", blurb: "npm, Cargo, Homebrew, direct downloads, and Windows batch launcher." },
    { title: "Changelog", href: "https://github.com/Hmbown/CodeWhale/blob/main/CHANGELOG.md", tag: "release", blurb: "Runtime stalls, sub-agent waits, MCP tool discovery, cache stability, and provider polish." },
    { title: "Roadmap", href: "/roadmap", tag: "0.9.0", blurb: "Follow-on work stays visible after the stabilization harvest." },
  ],
  movers: [],
};
...
const FALLBACK_DISPATCH_ZH: CuratedDispatch = {
  generatedAt: new Date().toISOString(),
  headline: "v0.8.48 发布加固——智能体活性、MCP 发现、IME 输入和 Windows 启动体验同步完成。",
  summary:
    "本「今日要闻」由 DeepSeek V4-Flash 每六小时重新生成。在 cron 写入真实仓库动态之前，下面的固定链接指向当前发布准备面。",
  highlights: [
    { title: "安装", href: "/zh/install", tag: "0.8.48", blurb: "npm、Cargo、Homebrew、直接下载和 Windows batch 启动器。" },
    { title: "变更日志", href: "https://github.com/Hmbown/CodeWhale/blob/main/CHANGELOG.md", tag: "release", blurb: "运行时卡死、子智能体等待、MCP 工具发现、缓存稳定性和 provider 打磨。" },
    { title: "路线图", href: "/zh/roadmap", tag: "0.9.0", blurb: "稳定性收割之后，后续工作继续保持可见。" },
  ],
  movers: [],
};

const RELEASE_HIGHLIGHTS_EN = [
  {
    title: "Sub-agent liveness",
    body: "Panic-safe completion guards and parent wait heartbeats prevent child-task failures from freezing release work.",
  },
  {
    title: "Input and status polish",
    body: "CJK/IME composer commits, animated compaction status, and wrapped statusline picker navigation are in the 0.8.48 set.",
  },
  {
    title: "MCP discovery",
    body: "Deferred tool search now returns 20 results by default, exposes max_results, and caps at 100 for dense MCP setups.",
  },
  {
    title: "Release artifacts",
    body: "Windows downloads now ship the shell-friendly codewhale.bat launcher beside the dispatcher and TUI executables.",
  },
];

const RELEASE_HIGHLIGHTS_ZH = [
  {
    title: "子智能体活性",
    body: "panic 安全的 completion guard 与父级等待心跳，避免子任务失败后冻结发布工作。",
  },
  {
    title: "输入与状态打磨",
    body: "CJK/IME composer 输入、压缩状态动效和 statusline 选择器循环导航已纳入 0.8.48。",
  },
  {
    title: "MCP 发现",
    body: "deferred tool-search 默认返回 20 条，支持 max_results，并对密集 MCP 配置限制到 100 条。",
  },
  {
    title: "发布产物",
    body: "Windows 下载现在随 dispatcher 和 TUI exe 一起包含便于 shell 启动的 codewhale.bat。",
  },
];
...
export default async function HomePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <>
      ...
      <section className="mx-auto max-w-[1400px] px-6 py-12 hairline-b">
        <div className="flex items-baseline gap-4 mb-6">
          <Seal char={isZh ? "稳" : "48"} />
          <div>
            <div className="eyebrow mb-2">{isZh ? "v0.8.48 发布准备" : "v0.8.48 release readiness"}</div>
            <h2 className="font-display text-3xl sm:text-4xl">
              {isZh ? "这版专注把长任务跑稳。" : "This release is about keeping long tasks moving."}
            </h2>
          </div>
        </div>
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-0 col-rule hairline-t hairline-b">
          {(isZh ? RELEASE_HIGHLIGHTS_ZH : RELEASE_HIGHLIGHTS_EN).map((item) => (
