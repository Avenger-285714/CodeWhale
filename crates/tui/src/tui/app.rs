...
impl App {
    ...
    pub fn new(options: TuiOptions, config: &Config) -> Self {
        ...
        let model = settings
            ...
            .unwrap_or_else(|| {
                // `model` (options.model) was derived from config.toml's
                // provider in main.rs. If `settings.default_provider` overrode
                // the active provider above (a runtime switch that survives
                // restarts), that fallback no longer matches the chosen
                // provider — so the chip would show e.g. MiMo while the footer
                // showed the OpenRouter default. Re-derive the default for the
                // *effective* provider so model, chip, and client all agree.
                // Only when `model` is still the config default — never clobber
                // an explicit `--model`.
                if provider != config.api_provider() && model == config.default_model() {
                    effective_auth_config.default_model()
                } else {
                    model
                }
            });
