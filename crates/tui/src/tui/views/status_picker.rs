...
impl ModalView for StatusPickerView {
    ...
    fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        let header = if self.rows.len() > visible_rows && !self.rows.is_empty() {
            format!(
            ...
            "Pick the chips you want in the footer:".to_string()
        };
            ...
            .take(visible_rows)
        {
            ...
            if is_cursor {
                ...
                let row_style = if checked {
                    Style::default().fg(palette::TEXT_PRIMARY)
                } else {
                    Style::default().fg(palette::TEXT_MUTED)
                };
                let hint_style = Style::default().fg(palette::TEXT_DIM);
...
mod tests {
    ...
    fn visible_row_start_keeps_cursor_in_view() {
        ...
        assert_eq!(visible_row_start(14, 13, 0), 0);
    }
