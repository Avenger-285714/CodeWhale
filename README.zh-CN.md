...
# 小米 MiMo（Token Plan — cn / sgp / ams 集群）
codewhale auth set --provider xiaomi --api-key "YOUR_MIMO_API_KEY"
codewhale --provider xiaomi --model mimo-v2.5-pro
# MIMO_CLUSTER=sgp codewhale --provider xiaomi     # 新加坡集群
# MIMO_BASE_URL="https://token-plan-ams.xiaomimimo.com/v1" codewhale --provider xiaomi  # 阿姆斯特丹集群
...
- 子智能体 fanout 增加 panic 安全的 completion guard 和父级等待心跳，避免子任务失败后卡死 turn loop。
- 周期性 context briefing 加入 120 秒超时，维护请求异常时会回退而不是无限阻塞会话。
- CJK/IME 输入现在直接进入 composer，同时保留快速多字符粘贴时的 Enter 抑制。
- `/statusline` 选择器支持首尾循环、保持光标可见，并让选中行横向铺满终端宽度。
- 发布产物现在随 Windows exe 一起包含 `codewhale.bat` 启动器，方便 shell 中直接启动。
...
| `CODEWHALE_PROVIDER` / `DEEPSEEK_PROVIDER` | `deepseek`（默认）、`nvidia-nim`、`openai`、`atlascloud`、`wanjie-ark`、`openrouter`、`novita`、`fireworks`、`xiaomi`、`sglang`、`vllm`、`ollama`（`moonshot` / `kimi` 于 v0.8.48 弃用，已有配置仍可加载） |
| `DEEPSEEK_PROFILE` | 配置 profile 名称 |
| `DEEPSEEK_MEMORY` | 设为 `on` 启用用户记忆 |
| `DEEPSEEK_ALLOW_INSECURE_HTTP=1` | 在可信网络上允许非本机 `http://` API base URL |
| `NVIDIA_API_KEY` / `OPENAI_API_KEY` / `ATLASCLOUD_API_KEY` / `WANJIE_ARK_API_KEY` / `OPENROUTER_API_KEY` / `NOVITA_API_KEY` / `FIREWORKS_API_KEY` / `MIMO_API_KEY` / `SGLANG_API_KEY` / `VLLM_API_KEY` / `OLLAMA_API_KEY` | 提供商认证 |
...
| `MIMO_BASE_URL` | 小米 MiMo 端点覆盖 |
| `MIMO_CLUSTER` | 小米 MiMo 区域集群：`cn`（默认）、`sgp`、`ams` |
...
> [!Note]
> V4 Pro 以上单价现为永久价格。DeepSeek 已于 2026 年 5 月 31 日将 75% 限时折扣转为永久性的四分之一基础费率调整。TUI 的成本估算已使用这些数值。后续价格变动请参阅官方 [DeepSeek 定价页面](https://api-docs.deepseek.com/zh-cn/quick_start/pricing)。
...
v0.8.48 稳定性收割中特别感谢 **[reidliu41](https://github.com/reidliu41)**、
...
**[buko](https://github.com/buko)** 和 **[T-Phuong-Nguyen](https://github.com/T-Phuong-Nguyen)** 的贡献。
...
- **[reidliu41](https://github.com/reidliu41)** — 退出后的恢复提示、工作区信任持久化、Ollama provider 支持、思考块流式终结修复、审批流程简化、模型选择器 Esc 修复、用户消息高亮、provider 选择器滚动、`/new` 会话命令、statusline 选择器导航、MCP shutdown 防护和 provider 帮助文案 (#863, #870, #921, #1078, #2143, #2056, #1995, #2241, #2235, #2324, #2357, #2366)
...
- **[donglovejava](https://github.com/donglovejava)** — paste @file 整合、CJK panic 修复、用户反馈、RLM 路由、edit_file 重试、压缩状态动效和 CJK/IME composer 输入修复 (#2154–#2168, #2302, #2330)
...
- **[cyq1017](https://github.com/cyq1017)** — /config 视图显示 base URL 和 Yuezhi provider 贡献 (#1967, #2375)
